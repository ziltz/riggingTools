...
class rig_tail( object ):
	...
	def make(self):
		...
		if self.makeDynamic:
			dynChain = rig_jointCopyChain(tailJnts[0], replaceName=('FK','Dyn') )
			finalChain = rig_jointCopyChain(tailJnts[0], replaceName=('FK','Final') )
			...
			ikData = pm.ikHandle(n='dynTail_ikHandle', sj=dynChain[0], ee=dynChain[-1],
			...
			pm.setAttr(ikData.handle + '.dWorldUpAxis', dWorldUpAxis)  # positive x
			...
			sc = rcon.simpleControls(dynChain, modify=1, constrainJoints=0, parentCon=1, colour='red' )
			pm.parent ( sc[dynChain[0]], tailModule.controlsSec )

			i = 0
			for jnt in sc:
				pm.parent( finalChain[i], sc[jnt] )
				i += 1
