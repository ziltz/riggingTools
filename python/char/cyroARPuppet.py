...
# puppet build
import char.cyroARPuppet as cyroAR_rp
reload(cyroAR_rp)
cyroAR_rp.build()

cyroAR_rp.buildScene()

'''

def buildScene():
	puppet( character='cyroAR',buildInScene=1 )

def build():
	puppet( character='cyroAR' )
	#puppet( character='cyro', rigBound = 'C:/Users/Jerry/Documents/maya/projects/cyro/scenes/rigBound/cyro_new_leg8.ma' )


def cyroARPrepareRig():
	print 'Prepare cyroAR'

	#rig_transform(0, name='headJAWorld', target='headJALocal_GRP')
	#rig_transform(0, name='headShapeWorld', target='headJALocal_GRP')
	allJoints = cmds.ls('*_JNT', type='joint')
	for jnt in allJoints:
		cmds.rename( jnt, jnt+'Drive' )

	cmds.duplicate( 'spineJA_JNTDrive', n='spineJA_JNTDrive1', rc=True )
	rigJoints = cmds.ls('*_JNTDrive1', type='joint')
	for jnt in rigJoints:
		cmds.rename( jnt, jnt.replace('Drive1', '') )
...
def cyroARRigModules():
	print 'Create cyroAR rig modules'
	...
	for side in ['l', 'r']:
		...
		cyroARShoulderUpgrade(side=side, ctrlSize=10)
	...
	tongueCtrls = fkControlChain( ('tongueJA_JNT', 'tongueJB_JNT','tongueJC_JNT','tongueJD_JNT'), 
								modify=0, lockHide=[], scale=[10,10,10], directCon=0)
	for t in tongueCtrls:
		pm.parent(t.offset, 'headControls_GRP')
	pm.parentConstraint('jawCon_GRP', tongueCtrls[0].offset, mo=True )
...
def cyroARFinish():
	...
	controlSet = pm.PyNode('cyroARRigPuppetControlSet')
	for s in ('l', 'r'):
	...
	driveJoints = cmds.ls('*_JNTDrive', type='joint')
	for jnt in driveJoints:
		rigJnt = jnt.replace('Drive', '')
		pm.parentConstraint( rigJnt, jnt, mo=True )

	pm.parent('spineJA_JNTDrive', 'rig_GRP')
	pm.rename('neckJEnd_JNT', 'neckJRig_JNT')
	pm.delete('*JEnd_JNT')
...
def cyroARShoulderUpgrade(side='', ctrlSize=1):
