...
class SkinSettings(object):
    def __init__(self, skinDeformer):
...
class PointWeights(object):
	...
	def getWeights(self,
	               ...
	               skinSettings,
	               shapeSettings):
		print 'pointCount = ' + str(shapeSettings.pointCount)
		for pointIndex in xrange(shapeSettings.pointCount):
			component = shapeSettings.getComponent(pointIndex)
			print 'component = ' + str(component)
			rawWeights = maya.cmds.skinPercent(skinSettings.skinDeformer,
												component,
			                                   q=True,
			                                   value=True)
			print 'rawWeights = ' + str(rawWeights) +'\n'
			return {weightIndex:weight for weightIndex, weight in enumerate(rawWeights) if weight > 0.0}
	...
	def saveWeights(self,
		...
		with open(targetFile, 'w') as skinFile:
			#for pointIndex in xrange(shapeSettings.pointCount):
			skinData = self.getWeights(inputMesh,skinSettings,shapeSettings)
				#skinFile.write(str(skinData))
			json.dump(skinData, skinFile)
...
class SkinIO(object):
    ...
    def export(self,
        ...
        for inputSkinNode in inputSkinNodes:
            skinSettings = SkinSettings(inputSkinNode)
